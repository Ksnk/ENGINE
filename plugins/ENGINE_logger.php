...
class ENGINE_logger
{
    ...
    static function _file_logger($msg, $arg = array())
    {
        $arg['{{date}}'] = date(DATE_RFC822);
        error_log(ENGINE::_t('{{date}} ' . $msg, $arg) . "\n", 3, self::$name);
    }

    static function _db_logger($msg, $arg = array())
    {
        if (!isset($arg['type'])) {
            $arg['type'] = 'userlog';
        }
    ...
    {
        $uri = parse_url(
            ENGINE::option('engine.logger_uri', 'file://logfile.log')
        ); // m 'db:///logtable
        if (!empty($uri['host'])) {
        ...
        switch ($uri['scheme']) {
            case 'file':
                if (!empty($uri['host'])) {
                    $uri['path'] = $uri['host'] . ENGINE::_($uri['path']);                } // so correct parse troubles
            ...
            case 'db': // so let's use current database for logging
                self::$name = ENGINE::option('engine.log_table', $uri['path']);
                ENGINE::db()->query(
