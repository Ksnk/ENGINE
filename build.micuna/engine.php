 ...
 * ------------------------------------------------------------------------------------------
 * $Id: X-Site cms (2.0, Micuna-bambolina build), written by Ksnk (sergekoriakin@gmail.com)
 *  Rev: 1410, Modified: 
        ...
        if (!isset(self::$class_list[$name])) {
            ...
            if (class_exists($class))
                ...
                if ($name == $class)
                    ...
                    self::error('class `{class}({name})` not found in (cwd:{dir})'
                        , array('{name}' => $name, '{class}' => $class, '{dir}' => getcwd()));
                return null;
            }
        }
...
// include all classes from `engine.include_files`
    ...
    static public function _report()
    {
        ...
        if (!empty(self::$db)) echo self::$db->report();
